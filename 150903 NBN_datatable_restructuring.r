...
long_to_wide <- function(the_file_name){
    ...
    file_saved <- paste(output_file_path , 'Wide_' , the_file_name  , sep = '')
    
    # Read the data into R
    data_full <- read.csv(file_name, header = TRUE, sep = "\t", dec = ",")
    ...
    write.table(data_closed_wide, file_saved, dec = "," , sep = "\t" , row.names = FALSE)
    cat(the_file_name, " has been saved in wide format." , "\n")
    cat("\n")
}
...
# source("C:\\R\\NBN_ad_hoc_analyses\\R_scripts\\150903 NBN_datatable_restructuring.r")
################################################################################################################
